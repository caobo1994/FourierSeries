...
Basic Operation.
\subsection{Syntax}
In the following section, \li{DerivT} is a function type $\mathbb{N}\rightarrow\text{FLOAT}$, and for any object \li{Deriv} of this type associated with a mathematical function, \li{Deriv(i)} gives the value of the \li{i}-th derivative of this mathematical function at the point $x=0$.
\subsubsection{Export Access Programs}
\begin{center}
\begin{tabular}{p{4cm} p{4cm} p{4cm} p{3cm}}
\hline
...
\noindent \li{Function(CFST CFS, DerivT Deriv)}:
\begin{itemize}
...
\begin{itemize}
	...
	\item Type $\mathbb{R}$ shall be implemented as \licpp{FLOAT}.
	\item Function types shall be implemented with \licpp{std::function}. For example, type $\mathbb{R}\rightarrow\mathbb{R}$ shall be implemented as \licpp{std::function<FLOAT(FLOAT)>}, while type $\mathbb{N}\rightarrow\li{FLOAT}$ shall be implemented as \licpp{std::function<FLOAT(size_t)>}. 
\end{itemize}
