...
\newcommand{\athecalnum}{C\thecalnum}
\newcommand{\calref}[1]{C\ref{#1}}
\newcounter{outputnum} %Output Number
\newcommand{\atheoutputnum}{O\theoutputnum}
...
\begin{itemize}
	...
	\item CFS: short for cut-off Fourier Series, a Fourier Series with finite number of non-zero weights.\an{Definition of CFS is given here.}
	\item Amplitude: a map $\mathit{CFS}\rightarrow \mathbb{R}^{*}$ to measure this CFS's strength.
\end{itemize}
\an{This section needs further expansion.}  \wss{Yes, I agree.  In particular,
	you haven't said what CFS stands for.  (Terms should not only be defined in
...
\begin{itemize}
	...
	function sets defined by this library (see \autoref{Appendix:BasicFunctions}), return the CFS $g(CFSf)$.\an{Basic functions refined in the appendix} 
	\an{I don't know whether to define the basic function set here or not. 
		I can, and I don't see any possible changes} \wss{The requirements should be
		complete, so you should define the basic function set.  I suggest introducing a
		data definition for the basic function set.  If the set should change, you can
		always document the change, or ``fake it''.}
...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		\hline
		Symbol &$\mathit{CFSFunc}: (\mathit{CFST}, \mathbb{R}\rightarrow\mathbb{R})\rightarrow\mathit{CFST}$\\
		\hline
		Equation& $\mathit{CFSFunc}(\mathit{CFSf}, g)=\mathit{CFS}(g(f(t)), n, \omega)$.\\
		\hline
...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		\hline
		Symbol &$\mathit{Amp}: \mathit{CFST}\rightarrow\mathbb{R}^{*}$\\
		\hline
...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		\begin{equation}
		\begin{aligned}
		%A_{f*g, i}&=A_{f,0}A_{g,0}+\frac{1}{2}\sum_{j=1}^{n}(A_{f,j}A_{g,j}+B_{f,j}B_{g,j}), i=0\\
...
\begin{itemize}
	...
	and must have corresponding Fourier series. \wss{I think you use this assumption
		in DD1.   You should invoke it there.  All assumptions need to be invoked
		somewhere, or else they aren't really assumptions that are being used.  This
		point is part of the SRS checklist.}\an{Done accordingly}
	
	\aitem{BasicFunction}
	Only the basic functions of CFS's are to be calculated. The aforementioned 
	basic functions are those in a library-defined set. \wss{As discussed elsewhere,
		you should explicitly define the functions that are basic.}
	%\item[A\refstepcounter{assumpnum}\theassumpnum \label{Ass:CFSPropertyMatch}:] 
	
	\oc{For any two-operand operations, the data $n$ and $\omega$ of these operands must be the same.}  
	\wss{Is this an assumption, or is it something that you can
		test?  From your MIS presentation, it looks like this is something you can test.}
	\an{When I wrote this down, I intended to assume this because of another design involving the arithmetic of $\omega$, but I later found out that that design is too complex for me, as well as not concrete mathematically, so I drop that, but I failed to delete this. I deleted this in the final version.}
	%\item[A\refstepcounter{assumpnum}\theassumpnum \label{Ass:Memory}:] 
	
	\oc{User shall allocate memory space for any variable other than CFS's in \iref{IM:ConvertTo} and \iref{IM:ConvertFrom}.}
	\wss{This assumption
		might be better in the design documentation.}
	\an{Noted and deleted.}
\end{itemize}
...
\wss{Great work Bo.  I have added some comments above, but overall you are
	definitely on the right track.  I have also made a few fixes directly, rather
	than ask you to make them.  You should review the diff so that you can see
	those fixes.}
\wss{My main comment is that you haven't really documented a CA; you have
	documented an SRS, but called it a CA.  In a CA you should identify different
	family members.  In particular, you need to know the parameters of variation
	for each variability.  Rather than cast your problem as a CA, I think it makes
	more sense to document your problem as an SRS.  I don't think it will take
	much for you to convert your document to follow the SRS template.  It will be
	a useful change though, because, as it is right now, the input, output,
	variability, etc headings are confusing.}
\an{Done accordingly}
\begin{table}
	\centering
