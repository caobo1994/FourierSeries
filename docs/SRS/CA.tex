...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		%SI Units & \si{\watt\per\square\metre}\\
		%\hline
		Equation& $App(CFSf, t_1)=\sum_{i=0}^{n}A_i\cos(i\omega t)+\sum_{i=1}^{n}B_{inf, i}\sin(i\omega t)$ \\
		\hline
...
\begin{minipage}{\textwidth}
	...
	\begin{tabular}{| p{\colAwidth} | p{\colBwidth}|}
		...
		\hline
		Symbol &$\mathit{CFSf}=\mathit{CFSg}$\\
		\hline
		% Units& $Mt^{-3}$\\
		% \hline
		%SI Units & \si{\watt\per\square\metre}\\
		%\hline
		Equation& Same as $\mathit{Amp}(\mathit{CFSf}-\mathit{CFSg})\leq \mathit{err}$, where $\mathit{err}$ is an user-given tolerance.\\
		\hline
